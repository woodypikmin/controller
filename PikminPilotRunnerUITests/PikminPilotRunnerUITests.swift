...
final class PikminPilotRunnerUITests: XCTestCase {
    ...
    func testPilotCommand() throws {
        ...
        if command == "select12" {
            selectPink(in: app, count: 12, interTapDelayUS: 35_000)
        ...
        // Stage 10.2 critical-tail macro. The host has already used DVT to
        // reveal and detect the pink filter. The whole pink -> count -> GO -> X
        // tail stays in one Runner session, but count (6...12) and pace are now
        // user-selectable from Pikmin Pilot.
        if command == "dispatchtail" {
            let requestedCount = Int(env["PIKMIN_PILOT_PINK_COUNT"] ?? "12") ?? 12
            let pinkCount = min(12, max(6, requestedCount))
            let fastMode = env["PIKMIN_PILOT_FAST_MODE"] == "1"

            let pinkSettle: useconds_t = fastMode ? 220_000 : 320_000
            ...
            selectPink(in: app, count: pinkCount, interTapDelayUS: interTap)
            ...
            guard let goPoint else {
                XCTFail("dispatchtail: active GO not detected after selecting \(pinkCount) pink Pikmin")
                return
            }
    ...
    private func selectPink(
        ...
        for (px, py) in points.prefix(min(12, max(6, count))) {
            app.coordinate(
