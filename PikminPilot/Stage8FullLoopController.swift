...
final class Stage8FullLoopController: ObservableObject {
    ...
    func start(
        ...
        emit("STAGE 8.1 FULL LOOP START • DVT screenshot + phone-local XCTest • WDA=OFF")
    ...
    private func beginBackgroundWindow(label: String) {
        ...
        backgroundTask = UIApplication.shared.beginBackgroundTask(
            withName: "PikminPilot-Stage8.1-\(label)",
    ...
    private func runOneDispatch(
        ...
        guard let close = try await waitForPoint(
            engine: engine,
            attempts: 50,
            delay: 0.38,
            stage: "carrying-green-x",
            detector: ImageAutomationDetector.detectCarryingClose
        ) else {
            throw LoopError("Round \(round): carrying green X not detected")
        }
        ...
        emit("ROUND \(round) • green X closed")
        await pause(0.68)
    }
    ...
    private func capture(
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("PikminPilot-Stage8.1-\(safeTag).png")
    ...
    private func ensureBackgroundBudget(
        ...
        if remaining.isFinite && remaining < 12.0 {
