...
final class Stage8FullLoopController: ObservableObject {
    ...
    func start(
        ...
        emit("STAGE 8.2 FULL LOOP START • single-session critical tail + persistent status • WDA=OFF")
    ...
    private func beginBackgroundWindow(label: String) {
        ...
        backgroundTask = UIApplication.shared.beginBackgroundTask(
            withName: "PikminPilot-Stage8.2-\(label)",
    ...
    private func runOneDispatch(
        ...
        if UIApplication.shared.applicationState != .active && remaining.isFinite {
            emit(String(format: "ROUND %d • background budget before critical tail = %.1fs", round, remaining))
        }
    ...
    private func capture(
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("PikminPilot-Stage8.2-\(safeTag).png")
