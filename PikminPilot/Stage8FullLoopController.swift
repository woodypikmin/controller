...
final class Stage8FullLoopController: ObservableObject {
    ...
    func start(
        ...
        emit("STAGE 8.1.2 FULL LOOP START • DVT screenshot + phone-local XCTest • WDA=OFF")
    ...
    private func beginBackgroundWindow(label: String) {
        ...
        backgroundTask = UIApplication.shared.beginBackgroundTask(
            withName: "PikminPilot-Stage8.1.2-\(label)",
    ...
    private func runOneDispatch(
        ...
        emit("ROUND \(round) • detect active GO")
        ...
        // The GO -> carrying transition is the longest uninterrupted foreground
        // segment in a dispatch. Renew early so Pilot remains alive long enough
        // to screenshot and close the carrying overlay.
        try await ensureBackgroundBudget(
            engine: engine,
            stage: "pre-GO",
            minimumRemaining: 24.0
        )
    ...
    private func waitForCarryingClose(
        ...
        // Stage 8.1 used a deliberately tight detector around the historical
        // lower-left X coordinate. On the real iOS 26 device the carrying X can
        // shift enough to fall outside that tiny window. Keep the strict test
        // first, then widen only inside the lower-left carrying-button ROI.
        for index in 0..<36 {
            try checkCancelled()
            if index % 4 == 0 {
                try await ensureBackgroundBudget(
            ...
            // After the transition has had time to settle, use the broader
            // geometry+green detector. It still cannot select the normal white
            // Expedition-list X.
            if index >= 3,
            ...
            // Stage 8.1.2: the user supplied a real carrying-screen screenshot
            // from the target iPhone (942 x 2048). The green X center measures
            // ~ (91.4, 1872.8), i.e. normalized (0.0971, 0.9144).
            //
            // The visual detector should normally win first. If DVT pixel/color
            // decoding still misses the button, do NOT wait forever: after a
            // short settling period, tap the screenshot-calibrated normalized
            // coordinate. This is intentionally restricted to the carrying-X
            // phase, after GO has already been confirmed and sent.
            if index >= 6 {
                let fixedPoint = CGPoint(
                    x: Double(image.cgImage?.width ?? 1) * 0.0971,
                    y: Double(image.cgImage?.height ?? 1) * 0.9144
                )
                emit("ROUND \(round) • carrying green X detector miss → screenshot-calibrated fallback x=0.0971 y=0.9144")
                return (fixedPoint, image, "fixed@0.0971,0.9144")
            }

            if index == 0 || index == 3 {
                emit("ROUND \(round) • carrying green X scan attempt \(index + 1)/36")
            }
            await pause(0.34)
        }
        return nil
    ...
    private func capture(
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("PikminPilot-Stage8.1.2-\(safeTag).png")
