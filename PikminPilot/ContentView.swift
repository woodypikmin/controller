            ...
            List {
                Section {
                    VStack(alignment: .leading, spacing: 7) {
                        ...
                        Text("Stage 8.2 — SINGLE-SESSION TAIL / PERSISTENT STATUS")
                            .font(.headline)
                ...
                Section("2. Phone-local Engine") {
                    ...
                    Button("STAGE 8.2 → START FULL LOOP") {
                        startStage81Loop()
                    }
                    .disabled(busy || loop.isRunning || pairing.pairingURL == nil)
                    ...
                    if busy || loop.isRunning {
                        HStack {
                            ProgressView()
                            Text(loop.isRunning ? "Stage 8.2 自動搬運中…" : "idevice 正在連線…")
                        }
                    }
                }
                ...
                Section("Stage 8.2 SINGLE-SESSION TAIL") {
                    Text("1. 開 LocalDevVPN。\n2. 先開 Pikmin Bloom，停在『探險水果列表』；不要 force quit。\n3. 回 Pikmin Pilot → CONNECT PHONE-LOCAL RSD。\n4. 按 STAGE 8.2 → START FULL LOOP。\n5. card-first：BUSY / COMPLETE 永遠不點，只選 AVAILABLE。\n6. DVT 偵測到粉紅後，時間敏感的『粉紅 → 12 → GO → 綠色 X』改成同一個 XCTest session，一次完成，避免 Pilot 在多次 XCTest bootstrap 中被 iOS 背景限制殺掉。\n7. 回水果列表後繼續下一個 AVAILABLE。\n8. Status 會持久化；即使 Pilot 被 iOS 重啟，重新打開也會保留上次 log。\n9. 只有真的按 STOP 才會寫 STOPPED BY USER；背景逾時會明確寫 reason=iOS-background-expired。")
                }
                ...
                Section("Bot Core") {
                    ...
                    Label("Stage 8.2：pink→12→GO→green-X single XCTest session + persistent status", systemImage: "arrow.triangle.2.circlepath")
                }
            }
