            ...
            List {
                Section {
                    VStack(alignment: .leading, spacing: 7) {
                        ...
                        Text("Stage 8.1.1 — GREEN-X LOOP FIX")
                            .font(.headline)
                ...
                Section("2. Phone-local Engine") {
                    ...
                    Button("STAGE 8.1.1 → START FULL LOOP") {
                        startStage81Loop()
                    }
                    .disabled(busy || loop.isRunning || pairing.pairingURL == nil)
                    ...
                    if busy || loop.isRunning {
                        HStack {
                            ProgressView()
                            Text(loop.isRunning ? "Stage 8.1.1 自動搬運中…" : "idevice 正在連線…")
                        }
                    }
                }
                ...
                Section("Bot Core") {
                    ...
                    Label("Stage 8.1.1：green-X detector + continuous loop", systemImage: "arrow.triangle.2.circlepath")
                }
            }
