...
// taps, swipes, and the configurable 6...12 pink-Pikmin selection batch.
...
async fn run_command(
    ...
    Ok(format!(
        "PHONE-LOCAL XCTEST COMMAND COMPLETED • command={} • p1=({:.5},{:.5}) • p2=({:.5},{:.5}) • duration={:.3} • pinkCount={} • fastMode={} • runner={} • pid={} • target={} • host={}",
...
unsafe fn execute_command_impl(
    ...
    if !(6..=12).contains(&pink_count) {
        write_message(message, message_capacity, "XCTest pink count must be within 6...12");
        return -124;
    }
