...
private let loopLimits:
    ...
    var body: some View {
        ZStack {
            NavigationStack {
                Form {
                    Section("Stage 6.2 - XCUITest Runner") {
                        Button(
                            "CHECK RUNNER :8200"
                        ) {
                            Task {
                        ...
                        Text("""
                        Stage 6.2 的 runner 是真正的 XCUITest process。
                        Runner 啟動後會在 localhost:8200 提供服務；Controller 本身不需要載入 XCTest。
                        """)
                        .font(.caption)
                    }
