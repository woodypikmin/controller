...
struct ContentView: View {
    ...
    @StateObject private var selfStartProbe =
        WDASelfStartProbe.shared
    ...
    @State private var wdaRunnerBundle =
        "com.facebook.WebDriverAgentRunner.xctrunner.9F34GUKYN4"
    ...
    var body: some View {
        ZStack {
            NavigationStack {
                Form {
                    Section("Stage 6.0 - Remove Windows") {
                        TextField(
                            "WDA Runner bundle id",
                            text:
                                $wdaRunnerBundle
                        )
                        ...
                        Text("""
                        有效測試條件：
                        localhost:8100 必須先是 DOWN。
                        這顆按鈕會嘗試直接從 iPhone 啟動已安裝的 WDA Runner，再等 8100 自己出現。
                        """)
                        .font(.caption)
                    }
