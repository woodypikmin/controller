...
struct DetectionResult {
    var fruits: [FruitCandidate]          // ONLY tappable AVAILABLE fruit
    var blockedObjects: [FruitCandidate] // fruit-like object but not safe
    ...
    static func classifyPlainObjects(
        ...
        for (rect, count) in components {
            ...
            let candidate = FruitCandidate(
                ...
                center: center,
                fill: fill,
                state: isKnownFruitLabel(label)
                    ? .available
                    : .blocked,
                labelText: label
            )

            // Stage 2.4 safety rule:
            // NO recognized fruit name = NEVER green.
